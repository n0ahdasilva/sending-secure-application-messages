...
def recv_msg(symmetric_key, data_block):
    """
    try:
        ...
        decr_symmetric_key = rsa.decrypt(symmetric_key, SERVER_PRV_KEY).decode("utf-8")

        # With the symmetric key, decrypt the data block.
        decr_data_block = Fernet(decr_symmetric_key).decrypt(data_block)
        print(decr_data_block)
        hash_digest_signature = decr_data_block[0]
        msg = decr_data_block[1]
    except:
    ...
    with open("server_private.pem", "rb") as f:
        SERVER_PRV_KEY = rsa.PrivateKey.load_pkcs1(f.read())
    # Decrypt the symmetric using the server's private key.
    decr_symmetric_key = rsa.decrypt(symmetric_key, SERVER_PRV_KEY)

    # With the symmetric key, decrypt the data block.
    decr_data_block = Fernet(decr_symmetric_key).decrypt(data_block).decode("utf-8")
    print(decr_data_block)
    hash_digest_signature = decr_data_block[0]
    msg = decr_data_block[1]
...
def main():
    ...
    while True:
        ...
        recv_msg(
            symmetric_key=client_socket.recv(1024),
            data_block=client_socket.recv(1024)
        )
